...
export class sinGrating2 extends Elements{
    ...
    async pre_draw() {
        ...
        if(!timeFrequency) {
            if(this.isNoise) {
                ...
                SGW.pre_singrat(this.ctx.canvas.id, sh.x-1.5*sh.r, sh.y-1.5*sh.r, sh.r,sh.pixelsPerDegree,sh.spatialFrequency,sh.angle,sh.contrast,sh.phase,sh.gamma);
